...
def create_mapping_configuration_file(graphid, include_concepts=True, data_dir=None):
    ...
    if include_concepts == True:
        ...
        dest = StringIO()
        dest.write(json.dumps(values, indent=4, sort_keys=False, ensure_ascii=False)) 
    ...
    dest = StringIO()
    #I ADDED HEBREW SUPPORT 
    dest.write(json.dumps(export_json, indent=4, sort_keys=False, ensure_ascii=False) )  
    files_for_export.append({'name':file_name, 'outputfile': dest})


    if data_dir != None:
        with open(os.path.join(data_dir), 'w') as config_file:
            json.dump(export_json, config_file, indent=4, sort_keys=False, ensure_ascii=False).encode('utf-8')
